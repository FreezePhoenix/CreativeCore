...
public class GuiParent extends GuiControl implements IGuiParent, Iterable<GuiChildControl> {
	...
	public boolean testForDoubleClick(Rect rect, double x, double y) {
		...
		y += -getOffsetY() - offset;
		for (GuiChildControl child : this)
			if (child.control.isInteractable() && child.control.testForDoubleClick(
					child.rect,
					x - child.getX(),
					y - child.getY()
			))
	...
	public boolean mouseClicked(Rect rect, double x, double y, int button) {
		...
		for (GuiChildControl child : this)
			if (!result && child.control.isInteractable() && child.isMouseOver(
					x,
					y
			) && child.control.mouseClicked(child.rect, x - child.getX(), y - child.getY(), button)) {
	...
	public boolean mouseDoubleClicked(Rect rect, double x, double y, int button) {
		...
		for (GuiChildControl child : this)
			if (!result && child.control.isInteractable() && child.isMouseOver(
					x,
					y
			) && child.control.mouseDoubleClicked(child.rect, x - child.getX(), y - child.getY(), button)) {
				raiseEvent(new GuiControlClickEvent(child.control, button, false));
				result = true;
			} else
				child.control.looseFocus();
	...
	public boolean mouseScrolled(Rect rect, double x, double y, double delta) {
		...
		y += -getOffsetY() - offset;
		for (GuiChildControl child : this)
			if (child.control.isInteractable() && child.isMouseOver(x, y) && child.control.mouseScrolled(
					child.rect,
					x - child.getX(),
					y - child.getY(),
					delta
