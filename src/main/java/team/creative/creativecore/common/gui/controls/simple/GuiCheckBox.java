...
public class GuiCheckBox extends GuiLabel {
	...
	protected void renderContent(PoseStack matrix, GuiChildControl control, Rect rect, int mouseX, int mouseY) {
		...
		style.get(ControlStyleFace.NESTED_BACKGROUND, rect.inside(mouseX, mouseY))
		     .render(matrix, 1, yoffset + 1, checkBoxWidth - 2, checkBoxWidth - 2);
		...
		if (value)
			Minecraft.getInstance().font.draw(
					...
					enabled
					? ColorUtils.WHITE
					: style.fontColorHighlight.toInt()
			);
		else if (partial)
