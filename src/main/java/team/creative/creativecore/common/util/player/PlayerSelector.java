...
public abstract class PlayerSelector {
	...
	static {
		...
		ConfigTypeConveration.registerSpecialType(
				(x) -> PlayerSelector.class.isAssignableFrom(x),
				new ConfigTypeConveration.SimpleConfigTypeConveration<PlayerSelector>() {
					...
					public JsonElement writeElement(PlayerSelector value, PlayerSelector defaultValue, boolean saveDefault) {
						return new JsonPrimitive(value.writeToNBT(new CompoundTag())
						                              .toString());
					}
					...
					public void createControls(GuiParent parent, Class clazz) {
						parent.add(new GuiPlayerSelectorButton(
								"data",
			...
			try {
				if (player instanceof ServerPlayer)
					return EntityArgument.players()
					                     ...
					                     .findPlayers(player.getServer().createCommandSourceStack())
					                     .contains(player);
				return true;
			} catch (CommandSyntaxException e) {
			}
