...
public class Rect {
	...
	public void scissor() {
		...
		RenderSystem.enableScissor(
				(int) Math.floor(realMinX),
				(int) Math.floor(realMinY),
				(int) Math.ceil(realMaxX),
				(int) Math.ceil(realMaxY) + 1
		);
	}
	...
	public Rect child(Rect rect, double scale, double xOffset, double yOffset) {
		return new Rect(
				minX + (rect.minX + xOffset) * scale,
				minY + (rect.minY + yOffset) * scale,
				minX + (rect.maxX + xOffset) * scale,
				minY + (rect.maxY + yOffset) * scale
		);
	}
