...
public class VectorFan {
	...
	protected void generate(RenderInformationHolder holder, Vec3f vec1, Vec3f vec2, Vec3f vec3, Vec3f vec4, List<BakedQuad> quads) {
		BakedQuad quad = new CreativeBakedQuad(
				holder.quad,
				holder.getBox(),
				holder.color,
				holder.shouldOverrideColor,
				holder.facing.toVanilla()
		);
		...
		for (int k = 0; k < 4; k++) {
			...
			if (holder.uvInverted) {
				uOffset = ((holder.facing.getV(oldX, oldY, oldZ) - holder.facing.getV(x, y, z)) / holder.facing.getV(
						holder.sizeX,
						holder.sizeY,
						holder.sizeZ
				)) * holder.sizeU;
				vOffset = ((holder.facing.getU(oldX, oldY, oldZ) - holder.facing.getU(x, y, z)) / holder.facing.getU(
						holder.sizeX,
						holder.sizeY,
						holder.sizeZ
				)) * holder.sizeV;
			} else {
				uOffset = ((holder.facing.getU(oldX, oldY, oldZ) - holder.facing.getU(x, y, z)) / holder.facing.getU(
						holder.sizeX,
						holder.sizeY,
						holder.sizeZ
				)) * holder.sizeU;
				vOffset = ((holder.facing.getV(oldX, oldY, oldZ) - holder.facing.getV(x, y, z)) / holder.facing.getV(
						holder.sizeX,
						holder.sizeY,
						holder.sizeZ
				)) * holder.sizeV;
	...
	public void renderPreview(Matrix4f matrix, BufferBuilder builder, float offX, float offY, float offZ, float scaleX, float scaleY, float scaleZ, int red, int green, int blue, int alpha) {
		...
		for (int i = 0; i < coords.length; i++) {
			Vec3f vec = coords[i];
			builder.vertex(matrix, vec.x * scaleX + offX, vec.y * scaleY + offY, vec.z * scaleZ + offZ)
	...
	public void renderLines(Matrix4f matrix, BufferBuilder builder, int red, int green, int blue, int alpha) {
		...
		while (index < coords.length - 3) {
			...
			builder.vertex(matrix, coords[index].x, coords[index].y, coords[index].z)
			       .color(red, green, blue, alpha)
			       .endVertex();
			builder.end();
			index += 2;
	...
	public boolean intersects(NormalPlane plane1, NormalPlane plane2) {
		...
		for (int i = 0; i <= coords.length; i++) {
			...
			if (i > 0)
				if (BooleanUtils.isTrue(one) != BooleanUtils.isTrue(beforeOne) && BooleanUtils.isTrue(two) != BooleanUtils.isTrue(
						beforeTwo)) {
	...
	public boolean intersect2d(VectorFan other, Axis one, Axis two, boolean inverse) {
		...
		for (int i = 1; i <= coords.length; i++) {
			...
			for (int i2 = 1; i2 <= other.coords.length; i2++) {
				Vec3f vec2 = i2 == other.coords.length
				             ? other.coords[0]
				             : other.coords[i2];
