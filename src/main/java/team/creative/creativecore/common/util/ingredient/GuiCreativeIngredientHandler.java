...
public abstract class GuiCreativeIngredientHandler {
	...
	public static GuiCreativeIngredientHandler defaultHandler = new GuiCreativeIngredientHandler() {
		...
		public void createControls(GuiParent gui, CreativeIngredient info) {
			GuiStackSelector selector = (GuiStackSelector) new GuiStackSelector(
					"inv",
					null,
					new GuiStackSelector.CreativeCollector(
							new GuiStackSelector.SearchSelector())
			).setExpandableX();
			gui.add(selector);
			...
			GuiStateButton damage = new GuiStateButton(
					"damage",
					0,
					new TextListBuilder().add("Damage: Off", "Damage: On")
			);
			gui.add(damage);
			...
			if (info instanceof CreativeIngredientBlock || info instanceof CreativeIngredientItem || info instanceof CreativeIngredientItemStack) {
				...
				if (info instanceof CreativeIngredientItemStack) {
					damage.nextState();
					if (((CreativeIngredientItemStack) info).needNBT) nbt.nextState();
				}
			}
		...
		public CreativeIngredient parseControls(GuiParent gui) {
			...
			if (stack != null) {
				...
				if (damage) {
					...
					else return new CreativeIngredientItem(stack.getItem());
				}
			}
			return null;
		}
	...
	static {
		...
		registerGuiInfoHandler("Material", new GuiCreativeIngredientHandler() {
			...
			@Override
			public void createControls(GuiParent gui, CreativeIngredient info) {
				GuiStackSelector selector = new GuiStackSelector(
						"inv",
						null,
						new GuiStackSelector.CreativeCollector(new GuiStackSelector.GuiBlockSelector())
				);
				selector.setExpandableX();
				gui.add(selector);
				if (info instanceof CreativeIngredientMaterial) selector.setSelectedForce(info.getExample());
			}
		...
		registerGuiInfoHandler("Blocktag", new GuiCreativeIngredientHandler() {
			...
			public CreativeIngredient parseControls(GuiParent gui) {
				...
				if (tag != null) return new CreativeIngredientBlockTag(tag);
				return null;
			}
			...
			public void createControls(GuiParent gui, CreativeIngredient info) {
				...
				GuiComboBoxMapped<TagKey<Block>> box = new GuiComboBoxMapped<>(
						"tag",
						new TextMapBuilder<TagKey<Block>>().addComponents(
								Registry.BLOCK.getTagNames()
								              .toList(),
								(TagKey<Block> x) -> {
									return new TextBuilder().stack(
											                        new ItemStack(
													                        Registry.BLOCK.getTag(
															                                x)
													                                      .get()
													                                      .get(0)
													                                      .value()))
				...
				if (info instanceof CreativeIngredientBlockTag) box.select(((CreativeIngredientBlockTag) info).tag);
			}
			...
			public void onChanged(GuiParent gui, GuiControlChangedEvent event) {
				if (event.control.is("search")) {
					GuiComboBoxMapped<TagKey<Block>> box = (GuiComboBoxMapped<TagKey<Block>>) gui.get("tag");
					box.setLines(new TextMapBuilder<TagKey<Block>>().setFilter(x -> x.toLowerCase()
					                                                                 .contains(((GuiTextfield) event.control).getText()))
					                                                .addComponents(Registry.BLOCK.getTagNames()
					                                                                             .toList(),
					                                                               (TagKey<Block> x) -> new TextBuilder().stack(
							                                                                                                     new ItemStack(Registry.BLOCK.getTag(
									                                                                                                                           x)
							                                                                                                                                 .get()
							                                                                                                                                 .get(0)
							                                                                                                                                 .value()))
		...
		registerGuiInfoHandler("Itemtag", new GuiCreativeIngredientHandler() {
			...
			public CreativeIngredient parseControls(GuiParent gui) {
				...
				if (tag != null) return new CreativeIngredientItemTag(tag);
				return null;
			}
			...
			public void createControls(GuiParent gui, CreativeIngredient info) {
				gui.flow = GuiFlow.STACK_Y;
				gui.align = Align.STRETCH;
				GuiComboBoxMapped<TagKey<Item>> box = new GuiComboBoxMapped<>(
						"tag",
						new TextMapBuilder<TagKey<Item>>().addComponents(
								Registry.ITEM.getTagNames()
								             .toList(),
								(TagKey<Item> x) -> new TextBuilder().stack(
										                                     new ItemStack(Registry.ITEM.getTag(
												                                     x).get().get(0)))
								                                     .text(x.location()
								                                            .toString())
								                                     .build()
				...
				gui.add(box);
				gui.add(new GuiTextfield("search"));
				if (info instanceof CreativeIngredientItemTag) box.select(((CreativeIngredientItemTag) info).tag);
			}
			...
			public void onChanged(GuiParent gui, GuiControlChangedEvent event) {
				if (event.control.is("search")) {
					GuiComboBoxMapped<TagKey<Item>> box = (GuiComboBoxMapped<TagKey<Item>>) gui.get("tag");
					box.setLines(new TextMapBuilder<TagKey<Item>>().setFilter(x -> x.toLowerCase()
					                                                                .contains(((GuiTextfield) event.control).getText()))
					                                               .addComponents(
							                                               Registry.ITEM.getTagNames()
							                                                            .toList(),
							                                               x -> new TextBuilder().stack(new ItemStack(
									                                                                     Registry.ITEM.getTag(
											                                                                             x).get()
									                                                                                  .get(0)))
							                                                                     .text(x.location()
							                                                                            .toString())
