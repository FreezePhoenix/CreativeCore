...
public abstract class CreativeIngredient {
	...
	static {
		...
		registerType("block", CreativeIngredientBlock.class, (x) -> {
			Block block = null;
			if (x instanceof Block) block = (Block) x;
			if (x instanceof BlockItem) block = Block.byItem((Item) x);

			if (block != null && !(block instanceof AirBlock)) return new CreativeIngredientBlock(block);
			return null;
		});
		registerType("blocktag", CreativeIngredientBlockTag.class, (x) -> {
			if (x instanceof TagKey tagKey && tagKey.registry().equals(Registry.BLOCK_REGISTRY)) {
				return new CreativeIngredientBlockTag((TagKey<Block>) tagKey);
			}
			return null;
		});
		...
		registerType("itemtag", CreativeIngredientItemTag.class, (x) -> {
			if (x instanceof TagKey tagKey && tagKey.registry().equals(Registry.ITEM_REGISTRY)) {
				return new CreativeIngredientItemTag((TagKey<Item>) tagKey);
			}
			return null;
		});

		registerType("itemstack",
		             CreativeIngredientItemStack.class,
		             (x) -> x instanceof ItemStack
		                    ? new CreativeIngredientItemStack((ItemStack) x, false)
		                    : null
		);

		registerType("material",
		             CreativeIngredientMaterial.class,
		             (x) -> x instanceof Material
		                    ? new CreativeIngredientMaterial((Material) x)
		                    : null
		);
		...
		ConfigTypeConveration.registerSpecialType((x) -> CreativeIngredient.class.isAssignableFrom(x),
		                                          new ConfigTypeConveration.SimpleConfigTypeConveration<CreativeIngredient>() {

			                                          @Override
			                                          public CreativeIngredient readElement(CreativeIngredient defaultValue, boolean loadDefault, JsonElement element) {
				                                          if (element.isJsonPrimitive() && ((JsonPrimitive) element).isString())
					                                          try {
						                                          return CreativeIngredient.load(TagParser.parseTag(
								                                          element.getAsString()));
			                                          ...
			                                          public void createControls(GuiParent parent, Class clazz) {
				                                          parent.add(new GuiInfoStackButton("data",
				                                                                            temp
				                                          ).setExpandable());
			                                          }
			                                          ...
			                                          public void loadValue(CreativeIngredient value, GuiParent parent) {
				                                          GuiInfoStackButton button = (GuiInfoStackButton) parent.get(
						                                          "data");
				                                          button.set(value);
			                                          }
	...
	public static <T extends CreativeIngredient> void registerType(String id, Class<T> classType, Function<Object, T> parser) {
		...
		if (parser != null) objectParsers.add(parser);
	}

	public static CreativeIngredient parse(Object object) {
		if (object == null) return null;
		if (object instanceof CreativeIngredient) return (CreativeIngredient) object;
	...
	public static CreativeIngredient load(CompoundTag nbt) {
		...
		if (classType == null) throw new IllegalArgumentException("'" + nbt.getString("id") + "' is an invalid type");
