...
public class ConfigTypeNamedList extends ConfigTypeConveration<NamedList> {
	...
	private ConfigHolderObject constructHolder(EnvType side, Object value) {
		return new ConfigHolderObject(
				fakeParent,
				(side == EnvType.CLIENT)
				? ConfigSynchronization.CLIENT
				: ConfigSynchronization.SERVER,
				"",
				value
		);
	}
	...
	@Override
	public NamedList readElement(NamedList defaultValue, boolean loadDefault, boolean ignoreRestart, JsonElement element, EnvType side,
			...
			for (Entry<String, JsonElement> entry : object.entrySet()) {
				if (conversation != null)
					addToList(
							list,
							entry.getKey(),
							conversation.readElement(
									ConfigTypeConveration.createObject(clazz),
									loadDefault,
									ignoreRestart,
									entry.getValue(),
									side,
					...
					holderConveration.readElement(
							constructHolder(side, value),
							loadDefault,
							ignoreRestart,
							entry.getValue(),
							side,
	...
	@Override
	public JsonElement writeElement(NamedList value, NamedList defaultValue, boolean saveDefault, boolean ignoreRestart, EnvType side,
	                                @Nullable ConfigKeyField key) {
		...
		for (Entry<String, ?> entry : (Set<Entry<String, ?>>) value.entrySet())
			if (conversation != null)
				array.add(
						entry.getKey(),
						conversation.writeElement(entry.getValue(), null, true, ignoreRestart, side, key)
				);
			else
				array.add(
						entry.getKey(),
						holderConveration.writeElement(
								constructHolder(side, entry.getValue()),
								null,
								true,
								ignoreRestart,
								side,
	...
	public void createControls(GuiParent parent, @Nullable ConfigKeyField key, Class clazz) {
		GuiListBoxBase<GuiConfigSubControl> listBox = (GuiListBoxBase<GuiConfigSubControl>) new GuiListBoxBase<>(
				"data",
				50,
				130,
				true,
		...
		parent.add(new GuiButton("add", x -> {
			...
			if (converation != null) {
				...
				ConfigHolderObject holder = constructHolder(EnvType.SERVER, value);
	...
	public void loadValue(NamedList value, GuiParent parent, @Nullable ConfigKeyField key) {
		...
		for (Entry<String, ?> entry : (Set<Entry<String, ?>>) value.entrySet()) {
			...
			if (converation != null) {
				...
				control = new GuiConfigSubControlHolder(
						"" + 0,
						constructHolder(EnvType.SERVER, entry.getValue()),
						entry.getValue()
				);
				((GuiConfigSubControlHolder) control).createControls();
				control.addNameTextfield(entry.getKey());
