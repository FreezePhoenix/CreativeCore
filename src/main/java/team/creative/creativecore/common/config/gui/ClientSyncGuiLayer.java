...
public class ClientSyncGuiLayer extends GuiLayer {
		...
		Function<ConfigKey, Collection<? extends ConfigKey>> getChildren = (x) -> {
			if (x.getDefault() instanceof ICreativeConfigHolder) {
				...
				for (ConfigKey key : ((ICreativeConfigHolder) x.getDefault()).fields())
					if (key.isWithoutForce(EnvType.CLIENT)) {
						Object object = key.get();
						if (!(object instanceof ICreativeConfigHolder) || !((ICreativeConfigHolder) object).isEmptyWithoutForce(
								EnvType.CLIENT))
							keys.add(key);
					}
				return keys;
	...
	public void load(CheckTree<ConfigKey>.CheckTreeEntry entry) {
		...
		ICreativeConfigHolder holder = entry.content == null
		                               ? root
		                               : (ICreativeConfigHolder) entry.content.get();

		add(new GuiLeftRightBox().addLeft(new GuiLabel("path").setTitle(new TextComponent("/" + String.join(
				                         "/",
				                         holder.path()
		                         ))))
		                         .addRight(new GuiButton("back", x -> load(entry.parent)).setTranslate("gui.back")
		                                                                                 .setEnabled(entry.parent != null)));
		...
		for (CheckTree<ConfigKey>.CheckTreeEntry key : currentView.children) {
			...
			String caption = translateOrDefault("config." + String.join(
					".",
					holder.path() + "." + key.content.name + ".name"
			), key.content.name);
			...
			if (key.content != null && key.content.get() instanceof ICreativeConfigHolder)
				...
				second.add(new GuiLabel(caption).setTitle(new TextComponent(caption))
