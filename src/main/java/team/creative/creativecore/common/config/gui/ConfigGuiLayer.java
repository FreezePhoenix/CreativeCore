...
public class ConfigGuiLayer extends GuiLayer {
	...
	public ConfigGuiLayer(ICreativeConfigHolder holder, EnvType side) {
	...
	public void loadHolder(ICreativeConfigHolder holder) {
		...
		for (ConfigKey key : holder.fields()) {
			...
			String caption = translateOrDefault(
					"config." + String.join(".", holder.path()) + "." + key.name + ".name",
					key.name
			);
	...
	public void sendUpdate() {
		if (side == EnvType.SERVER)
			...
			rootHolder.load(false, true, JsonUtils.get(ROOT, rootHolder.path()), EnvType.CLIENT);
			CreativeCore.CONFIG_HANDLER.save(EnvType.CLIENT);
		}
	}
