...
public abstract class ConfigHolder<T extends ConfigKey> implements ICreativeConfigHolder {
	...
	public void restoreDefault(EnvType side, boolean ignoreRestart) {
		for (int i = 0; i < fields.size(); i++) {
			...
			if (key.is(side) && (!ignoreRestart || !key.requiresRestart) && (!(key.get() instanceof ICreativeConfigHolder) || !((ICreativeConfigHolder) key.get()).isEmpty(
					side)))
		...
		for (int i = 0; i < fields.size(); i++) {
			...
			if (field.is(side) && (!ignoreRestart || !field.requiresRestart))
				if (json.has(field.name))
					field.set(ConfigTypeConveration.read(
							field.getType(),
							field.getDefault(),
							loadDefault,
							ignoreRestart,
							json
									.get(field.name),
							side,
							field instanceof ConfigKeyField
							? (ConfigKeyField) field
							: null
					));
				else if (loadDefault && (!(field.get() instanceof ICreativeConfigHolder) || !((ICreativeConfigHolder) field.get()).isEmpty(
						side)))
	...
	public JsonObject save(boolean saveDefault, boolean ignoreRestart, EnvType side) {
		...
		for (int i = 0; i < fields.size(); i++) {
			...
			if (field.is(side) && (!ignoreRestart || !field.requiresRestart) && (saveDefault || !field.isDefault(side)))
				object.add(field.name, ConfigTypeConveration.write(
						field.getType(),
						field.get(),
						field
								.getDefault(),
