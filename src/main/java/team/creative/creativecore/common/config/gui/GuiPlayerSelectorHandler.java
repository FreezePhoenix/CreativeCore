	...
	static {
		...
		REGISTRY.register("not", new GuiPlayerSelectorHandler<PlayerSelectorNot>() {
			...
			public void createControls(PlayerSelectorDialog gui, PlayerSelector selector) {
				gui.add(new GuiPlayerSelectorButton("not",
				                                    selector instanceof PlayerSelectorNot
				                                    ? ((PlayerSelectorNot) selector).selector
				                                    : new PlayerSelectorLevel(0)
				));
			}
		...
		REGISTRY.register("level", new GuiPlayerSelectorHandler<PlayerSelectorLevel>() {
			...
			@Override
			public void createControls(PlayerSelectorDialog gui, PlayerSelector selector) {
				gui.add(new GuiTextfield("content",
				                         selector instanceof PlayerSelectorLevel
				                         ? "" + ((PlayerSelectorLevel) selector).permissionLevel
				                         : "0"
				).setNumbersOnly());
			}
		...
		REGISTRY.register("mode", new GuiPlayerSelectorHandler<PlayerSelectorGamemode>() {
			...
			@Override
			public void createControls(PlayerSelectorDialog gui, PlayerSelector selector) {
				gui.add(new GuiStateButton("mode",
				                           selector instanceof PlayerSelectorGamemode
				                           ? ((PlayerSelectorGamemode) selector).type
						                           .getId()
				                           : 0,
				                           "survival",
				                           "creative",
		...
		REGISTRY.register("selector", new GuiPlayerSelectorHandler<PlayerSelectorCommandSelector>() {
			...
			public void createControls(PlayerSelectorDialog gui, PlayerSelector selector) {
				gui.add(new GuiTextfield("content",
				                         selector instanceof PlayerSelectorCommandSelector
				                         ? "" + ((PlayerSelectorCommandSelector) selector).pattern
				                         : "@a[]"
				));
			}
