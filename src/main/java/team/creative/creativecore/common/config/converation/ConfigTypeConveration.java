...
public abstract class ConfigTypeConveration<T> {
	...
	public static final ICreativeConfigHolder fakeParent = new ICreativeConfigHolder() {
		...
		@Override
		public JsonObject save(boolean saveDefault, boolean ignoreRestart, EnvType side) {
			return null;
		}
		...
		@Override
		public void load(boolean loadDefault, boolean ignoreRestart, JsonObject json, EnvType side) {
		}

		@Override
		public boolean isEmptyWithoutForce(EnvType side) {
			return false;
		}

		@Override
		public boolean isEmpty(EnvType side) {
			return false;
		}
	...
	static {
				...
				if (key != null) {
					if (decimal) {
						...
						if (intRange != null && intRange.slider()) {
							parent.add(new GuiSteppedSlider(
									"data",
									intRange.min(),
									intRange.min(),
									intRange.max()
		...
		registerType(SoundConfig.class, new ConfigTypeConveration<SoundConfig>() {
			...
			@Override
			public SoundConfig readElement(SoundConfig defaultValue, boolean loadDefault, boolean ignoreRestart, JsonElement element, EnvType side, ConfigKeyField key) {
				if (element.isJsonObject())
					return new SoundConfig(
							new ResourceLocation(element.getAsJsonObject().get("sound").getAsString()),
							element.getAsJsonObject().get("volume").getAsFloat(),
							element
			...
			@Override
			public JsonElement writeElement(SoundConfig value, SoundConfig defaultValue, boolean saveDefault, boolean ignoreRestart, EnvType side, ConfigKeyField key) {
		...
		holderConveration = registerType(ConfigHolderObject.class, new ConfigTypeConveration<ConfigHolderObject>() {
			...
			@Override
			public ConfigHolderObject readElement(ConfigHolderObject defaultValue, boolean loadDefault, boolean ignoreRestart, JsonElement element, EnvType side,
			                                      @Nullable ConfigKeyField key) {
				if (element.isJsonObject())
			...
			@Override
			public JsonElement writeElement(ConfigHolderObject value, ConfigHolderObject defaultValue, boolean saveDefault, boolean ignoreRestart, EnvType side,
			                                @Nullable ConfigKeyField key) {
		...
		registerType(ConfigHolderDynamic.class, new ConfigTypeConveration<ConfigHolderDynamic>() {
			...
			@Override
			public ConfigHolderDynamic readElement(ConfigHolderDynamic defaultValue, boolean loadDefault, boolean ignoreRestart, JsonElement element, EnvType side,
			                                       @Nullable ConfigKeyField key) {
				if (element.isJsonObject())
					defaultValue.load(loadDefault, ignoreRestart, (JsonObject) element, side);
				else
			...
			@Override
			public JsonElement writeElement(ConfigHolderDynamic value, ConfigHolderDynamic defaultValue, boolean saveDefault, boolean ignoreRestart, EnvType side,
			                                @Nullable ConfigKeyField key) {
				return value.save(saveDefault, ignoreRestart, side);
			}
		...
		registerSpecialType((x) -> x.isEnum(), new SimpleConfigTypeConveration<Enum>() {
			...
			@Environment(EnvType.CLIENT)
			public void createControls(GuiParent parent, Class clazz) {
				parent.add(new GuiComboBox(
						"data",
						new TextListBuilder().add(clazz.getEnumConstants(), (x) -> ((Enum) x).name())
				));
			}
	...
	public static Object read(Class typeClass, Object defaultValue, boolean loadDefault, boolean ignoreRestart, JsonElement element, EnvType side,
	                          @Nullable ConfigKeyField key) {
		return get(typeClass).readElement(defaultValue, loadDefault, ignoreRestart, element, side, key);
	}

	public static JsonElement write(Class typeClass, Object value, Object defaultValue, boolean saveDefault, boolean ignoreRestart, EnvType side,
	                                @Nullable ConfigKeyField key) {
	...
	public abstract T readElement(T defaultValue, boolean loadDefault, boolean ignoreRestart, JsonElement element, EnvType side,
	                              @Nullable ConfigKeyField key);

	public abstract JsonElement writeElement(T value, T defaultValue, boolean saveDefault, boolean ignoreRestart, EnvType side,
	                                         @Nullable ConfigKeyField key);
	...
	public static abstract class SimpleConfigTypeConveration<T> extends ConfigTypeConveration<T> {
		...
		@Override
		public T readElement(T defaultValue, boolean loadDefault, boolean ignoreRestart, JsonElement element, EnvType side,
		                     @Nullable ConfigKeyField key) {
		...
		@Override
		public JsonElement writeElement(T value, T defaultValue, boolean ignoreRestart, boolean saveDefault, EnvType side,
		                                @Nullable ConfigKeyField key) {
