...
public class ConfigTypeList extends ConfigTypeConveration<List> {
	...
	@Override
	public List readElement(List defaultValue, boolean loadDefault, boolean ignoreRestart, JsonElement element, EnvType side,
		...
		if (element.isJsonArray()) {
			...
			for (int i = 0; i < array.size(); i++)
				if (conversation != null)
					list.add(conversation.readElement(
	...
	@Override
	public JsonElement writeElement(List value, List defaultValue, boolean saveDefault, boolean ignoreRestart, EnvType side,
	                                @Nullable ConfigKeyField key) {
		...
		for (int i = 0; i < value.size(); i++)
			if (conversation != null)
				...
				array.add(holderConveration.writeElement(
						constructHolder(side, value.get(i)),
						null,
						true,
						ignoreRestart,
						side,
	...
	public void loadValue(List value, GuiParent parent, @Nullable ConfigKeyField key) {
		...
		for (int i = 0; i < value.size(); i++) {
			...
			if (converation != null) {
				...
				control = new GuiConfigSubControlHolder("" + 0, constructHolder(EnvType.SERVER, entry), entry);
				((GuiConfigSubControlHolder) control).createControls();
			}
