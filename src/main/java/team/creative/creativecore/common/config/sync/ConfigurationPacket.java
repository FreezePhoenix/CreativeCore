...
public class ConfigurationPacket extends CreativePacket {
	...
	public ConfigurationPacket(ICreativeConfigHolder holder, boolean ignoreRestart) {
		...
		this.json = holder.save(false, ignoreRestart, EnvType.SERVER);
	...
	public static void updateGui(Player player) {
		if (player != null && player.containerMenu instanceof IGuiIntegratedParent) {
			...
			if (((IGuiIntegratedParent) player.containerMenu).isOpen(ConfigGuiLayer.class) || ((IGuiIntegratedParent) player.containerMenu).isOpen(
					ClientSyncGuiLayer.class))
	...
	public void executeClient(Player player) {
		...
		if (holder != null)
			holder.load(true, ignoreRestart, json, EnvType.SERVER);
		updateGui(player);
	}
